...
public final class StereoStreamRenderer {
    private static final String TAG = "StereoStreamRenderer";
...
//    private OnFrameAvailableListener externalFrameListener;
    ...
    @Nullable
    private final GstVrSurfaceView gstVrSurfaceView;
    @Nullable
     ...
     */
    /* package */ StereoStreamRenderer(
            CanvasQuad canvasQuad, GstVrSurfaceView gstVrSurfaceView, Handler uiHandler,
            SurfaceTexture.OnFrameAvailableListener externalFrameListener) {
        this.canvasQuad = canvasQuad;
        this.gstVrSurfaceView = gstVrSurfaceView;
...
//        this.externalFrameListener = externalFrameListener;
     ...
     */
    @MainThread
    public static Pair<StereoStreamRenderer, GstVrSurfaceView> createForVR(Context context, ViewGroup parent) {
        CanvasQuad canvasQuad = new CanvasQuad();
        GstVrSurfaceView gstVrSurfaceView = GstVrSurfaceView.createForOpenGl(context, parent, canvasQuad);
        OnFrameAvailableListener externalFrameListener = gstVrSurfaceView.getFrameListener();

        StereoStreamRenderer scene = new StereoStreamRenderer(
                canvasQuad, gstVrSurfaceView, new Handler(Looper.getMainLooper()), externalFrameListener);
        return Pair.create(scene, gstVrSurfaceView);
    }
...
//        displayMesh.glDraw(viewProjectionMatrix, eyeType);
        if (gstVrSurfaceView != null) {
            canvasQuad.glDraw(viewProjectionMatrix, gstVrSurfaceView.getAlpha());
        }
